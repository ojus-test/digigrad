...
def build_assistant_prompt(spec: BusinessCallSpec, memory_digest: str = "") -> str:
    ...
    return (
        ...
        "call web_search first to find it (search the business name plus 'phone "
        "number', and the place/area when known); (2) call place_call with that number "
        "in E.164 (include the country code, e.g. +1 for US numbers), a one-or-two-"
        "sentence task describing exactly what to do, the business_name, and "
        "allow_booking=true if it's an order/booking/reservation; (3) ONLY after the "
        "tool returns success, tell them you're placing the call now and will text them "
        "the result. If the tool returns an error, tell them the call didn't go through "
        "— do not pretend it worked. If they ask you to 'find the X near Y and call the "
        "best/highest-rated one', use web_search to find the top-rated option AND its "
        "phone number, then place_call to that number — don't ask them to look it up "
        "themselves. If you know their location (e.g. a hotel) from memory, use it; "
